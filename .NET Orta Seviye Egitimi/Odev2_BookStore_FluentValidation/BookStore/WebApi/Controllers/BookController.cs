     ...
     */
     ...
     public IActionResult GetById(int id)
     {
        ...
        try
        {
          ...
          validator.ValidateAndThrow(query); //throws an error if there is a problem
          result = query.Handle();
        }
        catch (Exception ex)
        {
     ...
     public IActionResult AddBook([FromBody] CreateBookModel newBook)
     {
        ...
        */
        try
        {
          ...
          //**Model maplendikten sonra fluent validation ile bu model icin verilen validasyon kurallarina gore kontrol saglanir;
          CreateBookCommandValidator validator = new CreateBookCommandValidator(); //constructor icerisinde tanimlanmis olan rulelar yuklendi
          /*ValidationResult result = validator.Validate(command);
          if(!result.IsValid) //tum kurallardan gecemezse false doner ve buraya girer.
            foreach(var item in result.Errors)
               Console.WriteLine("Ozellik " + item.PropertyName + "- Error Message: " + item.ErrorMessage); //PropertyName,hangi field'in kontrolu yapilirken hata alindiysa onu doner.
          else //tum kurallardan gecerse true doner ve sorunsuz oldugundan bu kisim calisir
             command.Handle();
          */
          validator.ValidateAndThrow(command); //once validasyon yapip sonrasinda hatayi throw eder/firlatir, firlatilan mesaj da exception ile catch'te yakalanir, try-catch icerisinde yazmak gerekir yani.
          //Ustekki tum foreach ile donup yazdirdigin tum hatalari tekte hata olarak firlatti yani aslinda, ayni islevi gorur yukarisi ile.         
          command.Handle(); //validationda sorun yasanmazsa, ustteki kod hata firlatmazsa catch'e dusmez ve ebu calisir, post islemi gerceklesir, db'ye girilen kitap bilgileri eklenir.
        }
     ...
     public IActionResult UpdateBook(int id, [FromBody] UpdateBookModel updatedBook)
     {
        try
        {
         ...
         try-catch icerisine yazilir;*/
          ...
          validator.ValidateAndThrow(command); //throws an error if there is a problem
          command.Handle();
        }
        catch(Exception ex)
        {
